...
import { EventCard } from '@/app/(main)/event-calendar/ui/events';
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.success) {
               ...
               setError('error' in data ? (data.error as string) : 'Failed to search events');
               setSearchResults([]);
            }
          } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
            ...
            setSearchResults((prev) => [...prev, ...events]);
            setHasMore(data.hasMore);
        }
      }
    } catch (err) {
